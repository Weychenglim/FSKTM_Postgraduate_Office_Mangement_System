...
- Task 3 now provides transactional blank/copy/clone plan creation, Draft entry upsert, current-eligibility readiness validation, atomic publication/supersession, role-specific availability creation/cancellation, focused validation versus lifecycle conflict exceptions, and deterministic JSON-safe snapshots. Cross-semester copies retain their source through `supersedes` and COPY audit snapshots. Publication locks all and only same-semester plan entries in deterministic plan/Lecturer/entry order before readiness and supersession. Database uniqueness guards, stale-object checks, mandatory reasons, rollback, current-role copy filtering, Published/Superseded historical cloning, zero-limit publication, active-appointment preservation, and immutable per-action audits are covered by 19 focused lifecycle tests.
- Task 3 verification passes all 97 `academics.test_capacity` tests, `python manage.py check`, the migration dry run, new-service Black validation, and `git diff --check`.
- Capacity APIs, semester activation integration, migration baselines/seeding, appointment enforcement, reporting/reconciliation, and the frontend workspace remain planned.

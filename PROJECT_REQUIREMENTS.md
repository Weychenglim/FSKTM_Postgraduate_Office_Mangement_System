...
- Capacity-plan, capacity-audit, and availability-history lists retain array response bodies but require bounded `limit`/`offset` pagination. The default limit is 25, the maximum is 100, offset defaults to zero, and malformed, zero/negative, or excessive values return `400`. Successful list responses expose `X-Total-Count`, `X-Limit`, and `X-Offset`, including through configured CORS response-header exposure.

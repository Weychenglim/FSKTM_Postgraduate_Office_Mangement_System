...
- Dashboard page-level routing supports `/dashboard`, Office Staff/Admin-only `/dashboard/timeline`, and role-gated `/dashboard/reports` for Office Staff/Admin, Programme Coordinator, and Lecturer users; drawer, modal, filter, pagination, and selected-entry state inside Timeline Management stays component-local.

...
- `LecturerCapacityEntry` snapshots independent Supervisor and Panel limits. Model and QuerySet guards lock plans in deterministic order, permit writes and deletion only while every source and destination plan is Draft, and validate partial saves against projected persisted state. QuerySet deletion captures the exact locked and validated entry primary keys, then runs cascaded or raw deletion through a fresh primary-key-only queryset so mutable related-field predicates are never re-evaluated.

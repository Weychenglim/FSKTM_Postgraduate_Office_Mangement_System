...
- `src/components/CoordinatorDashboard.tsx` reuses the Lecturer dashboard composition for Programme Coordinators, loads the programme-scoped panel approval count from the coordinator workspace API, keeps Supervisor Approvals explicitly deferred, and uses the shared Lecturer-scoped timeline and next actions.
- `src/components/CoordinatorSupervisorDeferred.tsx` is the non-fabricated placeholder for the not-yet-persisted Programme Coordinator supervisor approval stage.
...
- Dashboard and panel appointment flows use `PortalToast` or inline field validation for user feedback instead of browser-default `alert()` dialogs.
...
- The office monitoring endpoint `/api/appointments/panel/` returns one row per meaningful workflow attempt. Each appointment is returned once, its underlying approved recommendation is omitted to prevent duplication, all other recommendation attempts remain visible, and a profile is returned as `No Panel` only when it has no appointment or recommendation. Every row has a stable `recordId` (`appointment-*`, `recommendation-*`, or `profile-*`), while `id` remains the student matric number. Rejected rows retain the recommended member, rejection stage/reason, and lifecycle timestamps used by the Office Staff/Admin detail page.
...
- Appointment transitions publish idempotent, non-announcement `Notification` rows with module, record type, record ID, and per-recipient event key inside the workflow transaction.
